...
def get_cai(tmp_wi_dict):
    ...
    tmp_seq_id_list = []
    tmp_cai_list = []
    fasta_seqs = SeqIO.parse(open(infile_name), 'fasta')
    ...
    for seq in fasta_seqs:
        ...
        for codon in codon_list:
            wi_list.append(float(tmp_wi_dict[codon]))
        if len(wi_list) > 5:
...
def output(tmp_seq_id_list, tmp_cai_list):
    """
    Output the results as a table
    """
    with open(outfile_name, 'w') as out:
        out.write('contig\tcai')
        for i in range(len(tmp_seq_id_list)):
            out_string = "{}\t{}".format(tmp_seq_id_list[i], tmp_cai_list[i])
            out.write('\n' + out_string)
...
if __name__ == '__main__':
    ...
    if exclude == 'yes':
        ...
        print("\nCalculating CAI normally with all amino acids included.")
    wi_dict = read_wi_table()
    (seq_id_list, cai_list) = get_cai(wi_dict)
    output(seq_id_list, cai_list)
    # return time to run

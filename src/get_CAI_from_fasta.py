...
from src.global_items import nucleotide_list, cpg_codon_list
...
def get_cai(input_file, wi_dict, exclude, debug):
    """
...
def output(output_file, seq_id_list, cai_list):
    ...
    with open(output_file, 'w') as out:
        ...
        for i in range(len(seq_id_list)):
            out_string = "{0}\t{1}".format(seq_id_list[i], cai_list[i])
            out.write('\n' + out_string)


def calculate_cai(input_file, output_file, wi_file, wi_column=3, exclude=None):
    ...
    if exclude == 'yes':
        ...
        print("\nCalculating CAI normally with all amino acids included.")
    wi_dict = read_wi_table(wi_file, wi_column - 1)
    (seq_id_list, cai_list) = get_cai(input_file, wi_dict, exclude, debug)
